...
def targeted_pgd(model, x, target, eps_n, alpha_n, steps, min_v, max_v, rand_start):
    ...
    for _ in range(steps):
        ...
        x_adv = x_adv - alpha_n * grad_sign  # negative gradient for targeted
...
def main():
    ...
    parser.add_argument("--out",    default="PGD-Targeted-Step-5.pt")
    ...
    for x, labs in loader:
        ...
        y_target = torch.randint_like(y_true, low=0, high=num_classes)
        mask = y_target == y_true
        while mask.any():
            y_target[mask] = torch.randint(0, num_classes, (mask.sum(),), device=device)
            mask = y_target == y_true
