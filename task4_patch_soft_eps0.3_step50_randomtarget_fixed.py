#!/usr/bin/env python3
"""
Task 4 – 32 × 32 *soft*‑patch **random‑targeted** PGD on ResNet‑34
• Blurred alpha mask (σ≈6) hides the hard square edge
• ε = 0.3, 50 steps (default), α = ε/steps  → subtle colour shift but strong attack
• Saves “Adversarial Test Set 3” ⟶  adv_set_patch_soft_random.pt

Run:
    python patch_pgd_random_target.py \
        --data   ./TestDataSet \
        --labels ./TestDataSet/labels_list.json

Any CLI flag can be overridden as usual (e.g. --eps 0.1 --steps 80).
"""
...
def patch_pgd(model, x, tgt, mask, eps_n, alpha_n, steps, min_v, max_v):
    """Momentum PGD constrained to the masked patch, **targeted** toward *tgt*."""
    x_adv = x.clone()
    mom = torch.zeros_like(x)          # momentum term
    beta = 0.9
    for _ in range(steps):
        x_adv = x_adv.detach().requires_grad_(True)
        loss = F.cross_entropy(model(x_adv), tgt)     # targeted loss
        loss.backward()

        grad = x_adv.grad.sign() * mask               # only inside patch
        mom = beta * mom + grad
...
def main():
    ...
    p.add_argument("--out",     default="adv_set_patch_soft_random_0.3_80.pt")
    args = p.parse_args()
    ...
    num_classes = len(ds.classes)  # usually 1000 for ImageNet
    ...
    for x, labs in loader:
        ...
        mask = gaussian_blur(hard, k=15, sigma=6).clamp(0, 1).repeat(1, 3, 1, 1)

        # ───── RANDOM target per sample (≠ ground‑truth) ─────
        true = torch.tensor([idx2true[int(l)] for l in labs], device=device)
        tgt  = torch.randint(0, num_classes, (B,), device=device)
        clash = tgt == true
        while clash.any():  # redraw until different
            tgt[clash] = torch.randint(0, num_classes, (clash.sum(),), device=device)
            clash = tgt == true
