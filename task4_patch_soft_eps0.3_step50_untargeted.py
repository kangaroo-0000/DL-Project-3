#!/usr/bin/env python3
"""
Task 4 – 32 × 32 *soft*‑patch untargeted PGD on ResNet‑34
• Blurred alpha mask (σ≈6) hides the hard square edge
• ε = 0.3, 80 steps, α = 0.006  →   subtle colour shift but strong attack
• Saves “Adversarial Test Set 3” ⟶  adv_set_patch_soft_untargeted.pt
"""
...
def patch_pgd(model, x, tgt, mask, eps_n, alpha_n, steps, min_v, max_v):
    ...
    for _ in range(steps):
        ...
        loss = F.cross_entropy(model(x_adv), tgt)  # untargeted
        loss.backward()
...
def main():
    ...
    p.add_argument("--out",    default="adv_set_patch_soft_eps0.3_step80_untargeted.pt")
    args = p.parse_args()
